...
private:
  ...
  void processSingleStatement(SourceLocation InsertLocationBeforeStatement,
                              const Stmt *Statement, InsertionMode Mode);
...
void LocalizingVariablesHandler::processCompoundStatement(
    const CompoundStmt *CompoundStatement) {
  processCompoundNestedScope(CompoundStatement->getBeginLoc(),
                             CompoundStatement);
}

void LocalizingVariablesHandler::processIfStatement(const IfStmt *IfStatement) {
  pushScope(IfStatement->getBeginLoc(), IfStatement);

  processSingleStatement(IfStatement->getBeginLoc(), IfStatement->getCond(),
                         InsertionMode::DeclarationAndAssignment);

  popScope();

  processNestedScope(IfStatement->getBeginLoc(), IfStatement->getThen());
  ...
  while (ElseStatement != nullptr) {
    if (const auto *ChainedIfStatement =
      ...
      pushScope(IfStatement->getBeginLoc(), ChainedIfStatement);

      processSingleStatement(IfStatement->getBeginLoc(),
                             ChainedIfStatement->getCond(),
                             InsertionMode::DeclarationAndAssignment);
      processNestedScope(IfStatement->getBeginLoc(),
      ...
      processNestedScope(IfStatement->getBeginLoc(), ElseStatement);
...
void LocalizingVariablesHandler::processForStatement(
  ...
  pushScope(ForStatement->getBeginLoc(), ForStatement);

  processSingleStatement(ForStatement->getBeginLoc(), ForStatement->getInit(),
                         InsertionMode::DeclarationAndAssignment);
  processSingleStatement(ForStatement->getBeginLoc(), ForStatement->getCond(),
                         InsertionMode::DeclarationOnly);
  processSingleStatement(ForStatement->getBeginLoc(), ForStatement->getInc(),
                         InsertionMode::DeclarationOnly);

  processNestedScope(ForStatement->getBeginLoc(), ForStatement->getBody());
...
void LocalizingVariablesHandler::processWhileStatement(
  ...
  pushScope(WhileStatement->getBeginLoc(), WhileStatement);

  processSingleStatement(WhileStatement->getBeginLoc(),
                         WhileStatement->getCond(),
                         InsertionMode::DeclarationAndAssignment);
  processNestedScope(WhileStatement->getBeginLoc(), WhileStatement->getBody());

  popScope();
}

void LocalizingVariablesHandler::processDoStatement(const DoStmt *DoStatement) {
  processSingleStatement(DoStatement->getBeginLoc(), DoStatement->getCond(),
                         InsertionMode::DeclarationOnly);
  processNestedScope(DoStatement->getBeginLoc(), DoStatement->getBody());
}

void LocalizingVariablesHandler::processForRangeStatement(
    const CXXForRangeStmt *ForRangeStatement) {
  processSingleStatement(ForRangeStatement->getBeginLoc(),
                         ForRangeStatement->getRangeStmt(),
                         InsertionMode::DeclarationOnly);
  processNestedScope(ForRangeStatement->getBeginLoc(),
                     ForRangeStatement->getBody());
}

void LocalizingVariablesHandler::processTryStatement(
    const CXXTryStmt *TryStatement) {
  processCompoundNestedScope(TryStatement->getBeginLoc(),
...
void LocalizingVariablesHandler::processCatchStatement(
  ...
  processNestedScope(CatchStatement->getBeginLoc(),
                     CatchStatement->getHandlerBlock());
}

void LocalizingVariablesHandler::processSwitchStatement(
    const SwitchStmt *SwitchStatement) {
  pushScope(SwitchStatement->getBeginLoc(), SwitchStatement);

  processSingleStatement(SwitchStatement->getBeginLoc(),
  ...
  if (CompoundBody != nullptr) {
    processSwitchStatementCompoundBody(SwitchStatement->getBeginLoc(),
                                       CompoundBody);
  } else {
    processSingleStatementNestedScope(SwitchStatement->getBeginLoc(),
                                      SwitchBody);
  }
...
void LocalizingVariablesHandler::processSingleStatementInSwitchBody(
  ...
  processSingleStatement(Statement->getBeginLoc(), Statement,
                         InsertionMode::DeclarationAndAssignment);
  Scopes[*CurrentVirtualScopeIndex].BraceEndInsertLocation =
      findLocationAfterSemi(Statement->getEndLoc(), Context);
}

void LocalizingVariablesHandler::processGenericStatement(
    const Stmt *GenericStatement) {
  processSingleStatement(GenericStatement->getBeginLoc(), GenericStatement,
                         InsertionMode::DeclarationAndAssignment);
}
...
void LocalizingVariablesHandler::processSingleStatementNestedScope(
  ...
  Scopes[NewScopeIndex].BraceStartInsertLocation =
      SingleStatementNestedScope->getBeginLoc();
  Scopes[NewScopeIndex].BraceEndInsertLocation =
      findLocationAfterSemi(SingleStatementNestedScope->getEndLoc(), Context);

  processSingleStatement(SingleStatementNestedScope->getBeginLoc(),
...
void LocalizingVariablesHandler::processDeclarationUsesInStatement(
  ...
  if (Mode == InsertionMode::DeclarationAndAssignment) {
    auto SimpleAssignmentMatcher = expr(ignoringCasts(binaryOperator(
        hasOperatorName("="),
        hasLHS(
            ignoringCasts(declRefExpr(to(varDecl().bind("assignedVarDecl"))))),
  ...
  for (const auto &Result : MatchResults) {
    ...
    if (VariableDeclaration == SimplyAssignedVariable) {
      ...
      NewVariableUseContext.InsertLocation = Statement->getBeginLoc();
    } else {
...
void LocalizingVariablesHandler::emitDiagnostics() {
  for (auto &LocalizedDeclaration : LocalizedDeclarations) {
    ...
    for (VariableDeclarationContext *VariableDeclaration :
         MovedVariableDeclarations) {
      auto Diagnostic =
          Check.diag(VariableDeclaration->Declaration->getBeginLoc(),
    ...
    if (DeclarationsToBeRemovedCount == TotalDeclarationsCount) {
      Diagnostic.AddFixItHint(
          FixItHint::CreateRemoval(CharSourceRange::getTokenRange(
              DeclarationStatement.Statement->getBeginLoc(),
              DeclarationStatement.Statement->getEndLoc())));
    } else {
      for (const auto &VariableDeclaration : VariableDeclarations) {
        if (VariableDeclaration.DeclarationStatementIndex ==
                DeclarationStatementIndex &&
          ...
          Diagnostic.AddFixItHint(
              FixItHint::CreateRemoval(CharSourceRange::getTokenRange(
                  ...
                  VariableDeclaration.Declaration->getEndLoc())));
        }
      }
    }
